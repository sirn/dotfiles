...
function printUsage(): void {
  console.log(`Usage: nix run nixpkgs#tsx ${process.argv[1]} <permissions.json> <command>`);
  console.log();
  console.log("Examples:");
  console.log('  nix run nixpkgs#tsx scripts/evaluate-shell-policy.ts ./plan-mode.json "ls -la"');
  console.log('  nix run nixpkgs#tsx scripts/evaluate-shell-policy.ts ./plan-mode.json \'bash -c "rm -rf /"\'');
}
...
function loadPolicy(jsonPath: string): { commands: PolicyCommands; wrappers?: WrapperRuleConfig[] } {
  ...
  const normalized = normalizeShellPolicyConfig(raw.commands ? raw : { commands: raw });
  ...
  for (const entry of entries) {
    ...
    switch (mode) {
      case "exact":
        ...
        break;
      case "prefix":
        matched = text.trimStart().toLowerCase().startsWith(match.toLowerCase());
        break;
      case "substring":
...
function main(): void {
  ...
  if (tokens.length === 0) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      console.log(`  ${GRY}${i + 1}.${R} ${YEL}${formatTokenValue(tokens[i])}${R}`);
    }
  }
  ...
  if (commands.length === 0) {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      const extra = cmd.redirects.length > 0 ? ` [${cmd.redirects.map(r => `${r.op} ${r.target}`).join(", ")}]` : "";
      process.stdout.write(`  ${GRY}${i + 1}.${R} ${YEL}${cmd.fullText}${R} ${formatSource(cmd.source)}${extra}\n`);
      ...
      if (allMatches.length > 0) {
        for (let j = 0; j < allMatches.length; j++) {
          ...
          process.stdout.write(`     ${branch}- ${B}match:${R} ${YEL}${m.match}${R}\n`);
          process.stdout.write(`     ${line} ${B}mode:${R} ${m.mode}\n`);
          process.stdout.write(`     ${line} ${B}policy:${R} ${formatAction(m.category)}\n`);
        }
      }
    }
  }
  ...
  // Final verdict
  const evalResult = evaluateCommand(args.commandToTest, policy.commands, wrapperRules);
