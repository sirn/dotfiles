...
The tool renders its result via `renderCall` and `renderResult` inside pi's default tool shell (no `renderShell: "self"`), matching how the subagent extension composes its tool box. `renderCall` returns an empty `Text` (zero lines), suppressing the redundant "update_goal status=..." title row — the result body header alone is self-explanatory. `renderResult` returns a transparent `Box(0,0)` (no own background; the shell provides `toolSuccessBg`/`toolErrorBg` framing) containing a colored bold header (" goal complete" / " goal blocked" / " goal update rejected") followed by the body: collapsed shows the objective (short) with an expand hint, expanded shows the full result text (with final usage when a budget is set). Error results use the `error` color for the header so rejected updates are distinguishable. The spacing matches `makeGoalBoxRenderer` exactly (header → 1 blank → body → 1 blank → hint).
