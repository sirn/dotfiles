...
in
{
  options.services.powerband = {
    ...
    coreUpThreshold =
      mkOpt lib.types.float
        "Per-core peak utilization (0.0–1.0) to go active (0 = disabled).";
    coreDownThreshold =
      mkOpt lib.types.float
        "Per-core peak utilization (0.0–1.0) to go idle (0 = disabled).";

    rqUpThreshold =
      mkOpt lib.types.float
        "Run-queue utilization (0.0–1.0) to go active (0 = disabled).";
    rqDownThreshold =
      mkOpt lib.types.float
        "Run-queue utilization (0.0–1.0) to go idle (0 = disabled).";

    pkgPowerUpThreshold =
      mkOpt lib.types.float
        "Package power (Watts) to go active (0 = disabled).";
    pkgPowerDownThreshold =
      mkOpt lib.types.float
        "Package power (Watts) to go idle (0 = disabled).";

    upDelay =
      mkOpt lib.types.ints.positive
        "Sustained seconds above up-threshold before switching to active.";
    downDelay =
      mkOpt lib.types.ints.positive
        "Sustained seconds below down-threshold before switching to idle.";
    ...
    systemd.services.powerband = {
      ...
      serviceConfig = {
        ...
        ExecStart =
          ...
          ++ lib.optionals (cfg.eppIdle != null) [ "--epp-idle" cfg.eppIdle ]
          ++ lib.optionals (cfg.eppActive != null) [ "--epp-active" cfg.eppActive ]
          ++ lib.optionals (cfg.epbIdle != null) [ "--epb-idle" (toString cfg.epbIdle) ]
          ++ lib.optionals (cfg.epbActive != null) [ "--epb-active" (toString cfg.epbActive) ]
          ++ lib.optionals (cfg.governorIdle != null) [ "--governor-idle" cfg.governorIdle ]
          ++ lib.optionals (cfg.governorActive != null) [ "--governor-active" cfg.governorActive ]
          ...
          ++ lib.optionals (cfg.upDelay != null) [ "--up-delay" (toString cfg.upDelay) ]
          ++ lib.optionals (cfg.downDelay != null) [ "--down-delay" (toString cfg.downDelay) ]
          ++ lib.optionals (cfg.pollInterval != null) [ "--poll-interval" (toString cfg.pollInterval) ]
