...
version=$(curl -sL "https://registry.npmjs.org/${pname}/latest" | jq -r '.version')
...
src_hash=$(nix hash convert --hash-algo sha256 --to sri \
  "$(nix-prefetch-url \
    "https://registry.npmjs.org/${pname}/-/gondolin-${version}.tgz" \
    2>/dev/null | tail -1)")
...
curl -sL "https://registry.npmjs.org/${pname}/-/gondolin-${version}.tgz" |
...
build_log=$(nix build --no-link --print-out-paths "path:${repo_root}#gondolin.npmDeps" 2>&1 || true)
npm_deps_hash=$(echo "$build_log" | grep 'got:' | head -1 | sed 's/.*got: *//')

if [ -z "$npm_deps_hash" ]; then
  echo "ERROR: Failed to determine npmDeps hash"
  nix build --no-link "path:${repo_root}#gondolin.npmDeps" 2>&1 | tail -10
  exit 1
fi
