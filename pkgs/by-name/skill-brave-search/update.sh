...
commit_json=$(curl -sL https://api.github.com/repos/brave/brave-search-skills/commits/main)
...
src_hash=$(nix hash convert --hash-algo sha256 --to sri \
  "$(nix-prefetch-url --unpack "https://github.com/brave/brave-search-skills/archive/${rev}.tar.gz" 2>/dev/null | tail -1)")
...
nix-build -E 'let pkgs = import <nixpkgs> {}; in pkgs.callPackage ./pkgs/by-name/skill-brave-search {}' \
  --no-out-link
