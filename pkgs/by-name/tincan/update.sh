...
# Set a fake vendorHash so the Go module vendor staging derivation fails and
# reports the correct fixed-output hash. The vendor staging only downloads
# modules; it does not compile any Go code.
sed_inplace 's|vendorHash = "sha256-[^"]*"|vendorHash = "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="|' "$package_file"

build_log=$(nix build --no-link "path:${repo_root}#${pname}.goModules" 2>&1 || true)
vendor_hash=$(echo "$build_log" | grep 'got:' | head -1 | sed 's/.*got: *//')

if [ -z "$vendor_hash" ]; then
  echo "ERROR: Failed to determine vendorHash"
  echo "$build_log" | tail -20
  exit 1
fi

sed_inplace "s|vendorHash = \"sha256-[^\"]*\"|vendorHash = \"$vendor_hash\"|" "$package_file"
